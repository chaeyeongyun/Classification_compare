...
def train(opt):
    ...
    start_epoch, load_model = opt.start_epoch, opt.load_model
    ...
    if model_name == 'resnet':
        optimizer = torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.9, dampening=0, weight_decay=0.0001, nesterov=False) 
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if test_acc > last_test_acc:
            torch.save(model.state_dict(), f'./save_files/{epoch}_{num_epochs}ep_{batch_size}b_best.pt')
    ...
    torch.save(model.state_dict(), f'./save_files/{num_epochs}ep_{batch_size}b_final.pt')
